...
{
    // Used as the width hint when the widget has not yet been laid out (width == 0).
    // Large enough that no elision occurs at that stage.
    static constexpr int ELIDE_UNCONSTRAINED_WIDTH = 9999;
...
private:
    [[nodiscard]] QString elided() const
    {
        return fontMetrics().elidedText(m_fullText, Qt::ElideRight,
                                        width() > 0 ? width() : ELIDE_UNCONSTRAINED_WIDTH);
    }
